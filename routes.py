...
def login():
    if request.method == 'POST':
        ...
        porteiro = get_porteiro_by_username(username)
...
def registrar_encomenda():
    if request.method == 'POST':
        ...
        if wa_link:
            ...
            flash(f'Encomenda registrada com ID {shortened_id}. Envie este ID ao morador.')
        return redirect(url_for('main.dashboard'))
...
def cadastrar_porteiro_route():
    if request.method == 'POST':
        ...
        username = request.form.get('username')
        password = request.form.get('password')
        if get_porteiro_by_username(username):
            flash('Já existe um porteiro com esse usuário.')
        else:
            create_porteiro(nome, username, password)
