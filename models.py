...
class Porteiro(db.Model):
    ...
    encomendas = db.relationship(
        "Encomenda", foreign_keys="Encomenda.porteiro_id", backref="porteiro", lazy=True
    )
...
class Encomenda(db.Model):
    ...
    retirada_porteiro_id = db.Column(
        db.Integer, db.ForeignKey("porteiros.id"), nullable=True
    )
