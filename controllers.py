...
def get_porteiro_by_username(username):
...
def create_porteiro(nome, username, senha):
    hashed = generate_password_hash(senha)
    db.session.execute(
        text('INSERT INTO porteiros (nome, username, password_hash) VALUES (:nome, :username, :pwd)'),
        {'nome': nome, 'username': username, 'pwd': hashed}
    )
    db.session.commit()
...
def get_morador_by_unique(nome, torre, ap):
    result = db.session.execute(
        text('SELECT * FROM moradores WHERE nome = :nome AND torre = :torre AND ap = :ap'),
...
def create_morador(nome, torre, ap, whatsapp):
    db.session.execute(
        text('INSERT INTO moradores (nome, torre, ap, whatsapp) VALUES (:nome, :torre, :ap, :whatsapp)'),
...
def finalize_retirada(encomenda_id, morador_nome, porteiro_id):
    ...
    morador = db.session.execute(
        text('SELECT * FROM moradores WHERE nome = :nome LIMIT 1'), {'nome': morador_nome}
    ).fetchone()
    if morador:
        ...
        db.session.commit()
        morador_id = db.session.execute(text('SELECT last_insert_rowid()')).fetchone()[0]
    now = datetime.utcnow()
    db.session.execute(
        text(
